...
export async function createEvent(eventData: Omit<Event, 'user_id'>) {
  console.log('createEvent function called with data:', JSON.stringify(eventData, null, 2));
  ...
  try {
    ...
    if (userError) {
      ...
      throw { message: 'Authentication error: ' + userError.message, code: 'AUTH_ERROR' };
    }
    
    if (!user) {
      console.error('No user found');
      throw { message: 'User must be logged in to create an event', code: 'AUTH_REQUIRED' };
    }
      ...
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        ...
        if (altError) {
          ...
          if (genericError) {
            ...
            console.log('All table attempts failed. Storing locally and simulating success.');
            const localEvent = {
              ...eventPayload,
              id: 'local_' + Date.now(),
              success: true,
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        ...
        if (altError) {
          ...
          console.log('Returning combined sample and local events for UI to display...');
    ...
    if (data && data.length > 0) {
      data.forEach(dbEvent => {
        ...
        const existsInLocal = localEvents.some(localEvent => {
          ...
          return dbEvent.title === localEvent.title && 
...
export async function getUserEvents() {
  try {
    ...
    const localUserEvents = localEvents.filter(event => event.user_id === user.id);
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        ...
        if (altError) {
          ...
          return getSampleEvents().find(e => e.id === id) || {
            id,
